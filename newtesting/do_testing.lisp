...
(defparameter *previous-match* nil)
...
(defmethod low-level-lookup-seclevel ((onions onion-state) database table field onion)
  (let ((table-data (cdr (str-assoc database (onion-state-tables onions)))))
    (assert table-data)
...
(defmethod handle-check (connections (onions onion-state) (type (eql :all-max)) onion-check)
...
(defmethod handle-check (connections (onions onion-state) (type (eql :update)) onion-check)
  (update-onion-state! onions onion-check)
  (handle-check connections onions :check onion-check)) 

(defmethod handle-check (connections (onions onion-state) (type (eql :check)) onion-check)
...
(defun run-tests (all-test-groups)
  ...
  (let ((connections (make-connection-state :cryptdb nil :plain nil))
        (results '()))
...
(defun test-lookup-seclevel ()
  (let ((onions (make-onion-state :tables '(("database"
